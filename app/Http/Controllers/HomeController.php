...

{
...

    {
        ...
        $services = SistasService::where('status',1)->get()->take($serviceVisibilty->qty);
    ...
    public function aboutUs(){
        ...
        $services = SistasService::where('status',1)->get();
    ...
    public function service(Request $request){
        ...
        $services = Service::with('activeVariants.activeVariantItems')->orderBy('id','desc')->where(['status' => 1, 'approve_by_admin' => 1]);
        ...
        if($request->child_category) {
            ...
            $services = $services->where('child_category_id', $child_category->id);
        ...
        if($request->highlight){
            if($request->highlight == 'popular_category'){
                ...
                $services = $services->whereIn('category_id', $popularCategoryArr);
            ...
            if($request->highlight == 'top_product'){
                ...
                $services = $services->where('is_top',1);
            ...
            if($request->highlight == 'new_arrival'){
                ...
                $services = $services->where('new_product',1);
            ...
            if($request->highlight == 'featured_product'){
                ...
                $services = $services->where('is_featured',1);
            ...
            if($request->highlight == 'best_product'){
                ...
                $services = $services->where('is_best',1);
        ...
        if($request->brand) {
            ...
            $services = $services->where('brand_id', $brand->id);
        ...
        $services = $services->select('id','name', 'short_name', 'slug', 'thumb_image','qty','sold_qty', 'price', 'offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id');

        $services = $services->paginate($paginateQty);
