      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="section-body">
            <a href="{{ route('seller.service.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> {{__('admin.Add New')}}</a>
            <div class="row mt-4">
                <div class="col">
                  <div class="card">
                    <div class="card-body">
                        ...
                        <table class="table table-striped" id="dataTable">
                            ...
                            <tbody>
                                @foreach ($services as $index => $service)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td><a target="_blank" href="{{ $frontend_url.$service->slug }}">{{ $service->short_name }}</a></td>
                                        <td>{{ $setting->currency_icon }}{{ $service->price }}</td>
                                        <td> <img class="rounded-circle" src="{{ asset($service->thumb_image) }}" alt="" width="100px" height="100px"></td>
                                        <td>
                                            @if ($service->new_service == 1)
                                            ...
                                            @if ($service->is_best == 1)
                                            <span class="badge badge-danger p-1">{{__('admin.Best Service')}}</span>
                                            @endif
                                      ...
                                      {{-- Start This code is for check for exixst order and stop the admin to delete the service --}}
                                        {{-- @php
                                            $existOrder = $orderservices->where('service_id',$service->id)->count();
                                        @endphp --}}
                                      ...
                                      {{-- End This code is for check for exixst order and stop the admin to delete the service --}}
                                        ...
                                        {{-- <div class="dropdown d-inline">
                                            ...
                                            <div class="dropdown-menu" x-placement="top-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, -131px, 0px);">
                                              ...
                                              <a class="dropdown-item has-icon" href="{{ route('admin.service-variant',$service->id) }}"><i class="fas fa-cog"></i>{{__('admin.service Variant')}}</a>
          ...
          <div class="modal-dialog" role="document">
              <div class="modal-content">
                        <div class="modal-body">
                            {{__('admin.You can not delete this service. Because there are one or more order has been created in this service.')}}
                        </div>
